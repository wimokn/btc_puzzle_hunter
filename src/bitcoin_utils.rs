...
// Legacy address generation using raw crypto (for reference/testing)
pub fn private_key_to_legacy_address(private_key: &BigUint, compressed: bool) -> Result<String> {
    let secp = Secp256k1::new();
    let key_bytes = private_key_to_bytes(private_key)?;
    let secret_key = SecretKey::from_slice(&key_bytes)?;
    let public_key = PublicKey::from_secret_key(&secp, &secret_key);
    ...
    let mut full_hash = versioned_hash;
...
mod tests {
    ...
    fn test_known_private_key() {
        ...
        assert!(addresses.contains(&"1BgGZ9tcN4rm9KBzDn7KprQz87SZ26SAMH".to_string()) ||
